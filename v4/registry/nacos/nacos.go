...
func (n *nacosRegistry) Register(s *registry.Service, opts ...registry.RegisterOption) error {
	...
	if options.Context != nil {
		ctx = options.Context
	} else if n.opts.Context != nil {
		ctx = n.opts.Context
	}
	if ctx != nil {
		if p, ok := ctx.Value("register_instance_param").(vo.RegisterInstanceParam); ok {
			param = p
			withContext = ok
		}
	}
...
func (n *nacosRegistry) Deregister(s *registry.Service, opts ...registry.DeregisterOption) error {
	...
	if ctx != nil {
		if p, ok := ctx.Value("deregister_instance_param").(vo.DeregisterInstanceParam); ok {
			param = p
			withContext = ok
		}
	}
...
func (n *nacosRegistry) GetService(name string, opts ...registry.GetOption) ([]*registry.Service, error) {
	...
	if ctx != nil {
		if p, ok := ctx.Value("select_instances_param").(vo.GetServiceParam); ok {
			param = p
			withContext = ok
		}
	}
...
func (n *nacosRegistry) ListServices(opts ...registry.ListOption) ([]*registry.Service, error) {
	...
	if ctx != nil {
		if p, ok := ctx.Value("get_all_service_info_param").(vo.GetAllServiceInfoParam); ok {
			param = p
			withContext = ok
		}
	}
